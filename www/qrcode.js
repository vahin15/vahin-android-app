/**
 * Lightweight, zero-dependency QR Code Generator & Scanner Helper for Unifest
 */
(function (global) {
  'use strict';

  // Minimal QR code generator based on standard QR matrix specification
  ...
  // Helper to draw the QR code on a Canvas element
  var QRCodeGen = {
    draw: function (text, canvas, options) {
      if (!canvas) return;
      options = options || {};
      var size = options.size || 220;
      var darkColor = options.darkColor || '#171A21';
      ...
      if (text.length > 100) typeNum = 8;

      var qr;
      for (var t = 2; t <= 10; t++) {
        try {
          qr = new QRCodeModel(t, 1); // Level M
          qr.addData(text);
          qr.make();
          break;
        } catch (e) {
      ...
      for (var row = 0; row < count; row++) {
        for (var col = 0; col < count; col++) {
          if (qr.isDark(row, col)) {
            var x = Math.round(margin + col * cellSize);
            var y = Math.round(margin + row * cellSize);
      ...
      if (options.logoText) {
        ...
        ctx.fillText(options.logoText, size / 2, size / 2 + 1);
      }
    }
  };
